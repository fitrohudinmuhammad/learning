DROP FUNCTION IF EXISTS create_journal_leasing(int, int);
CREATE OR REPLACE FUNCTION public.create_journal_leasing(line_id int, leas_account_id int)
RETURNS INT
LANGUAGE plpgsql AS
$$
    DECLARE
    ...
    BEGIN
        ...
        IF leasing_account_ids.id IS NOT NULL THEN
            ...
            name_sequence := get_sequence_name_journal(leasing_account_ids.account_journal_id, (leasing_ids.order_date + interval '7 hour')::varchar);
            ...
            INSERT INTO account_move(
                ...
                nextval('account_move_id_seq'), name_sequence, leasing_ids.debtor_id, (leasing_ids.order_date + interval '7 hour')::date, leasing_account_ids.account_journal_id,
                2, 1, currency, concat(leasing_account_ids.code, '/', date_part('year', (leasing_ids.order_date + interval '7 hour')::date), '/', date_part('month', (leasing_ids.order_date + interval '7 hour')::date), '/'),
                'entry', '', 0.00, 0.00, leasing_ids.amount,
                0.00, 0.00, leasing_ids.amount, 0.00,
                (leasing_ids.order_date + interval '7 hour')::date, 'posted', 'not_paid', 'no',
            ...
            INSERT INTO account_move_line(
                ...
                (leasing_ids.order_date + interval '7 hour')::date, leasing_ids.amount, 0.00,
            ...
            INSERT INTO account_move_line(
                ...
                name_sequence, 'posted', concat(feetypeids.name, ' - ', channelids.name, ' - ', accountno.name->>'en_US', ' - ', paychan.name), 'product',
                (leasing_ids.order_date + interval '7 hour')::date, 0.00, leasing_ids.amount,
...
DROP FUNCTION IF EXISTS create_journal_payment(int, int);
CREATE OR REPLACE FUNCTION public.create_journal_payment(line_id int, leas_account_id int)
RETURNS INT
LANGUAGE plpgsql AS
$$
    DECLARE
...
--select create_journal(229, 3, 12, 'SRV/2023/10/0002');
---------------------------

DROP FUNCTION IF EXISTS create_journal_interest_eom(int, int);
CREATE OR REPLACE FUNCTION public.create_journal_interest_eom(line_id int, leas_account_id int)
RETURNS INT
LANGUAGE plpgsql AS
$$
    DECLARE
    ...
    BEGIN
        ...
        IF leasing_account_ids.id IS NOT NULL THEN
            ...
            INSERT INTO account_move(
                ...
                (leasing_ids.order_date + interval '7 hour')::date, case when (leasing_ids.order_date + interval '7 hour')::date < create_dates::date then 'posted' else 'draft' end, 
                'not_paid', case when (leasing_ids.order_date + interval '7 hour')::date < create_dates::date then 'no' else 'at_date' end,
                'no_extract_requested', 2, create_dates, 2, create_dates,
            ...
            INSERT INTO account_move_line(
                ...
                nextval('account_move_line_id_seq'), account_move_id, leasing_account_ids.account_journal_id, 1, currency, leasing_account_ids.debit_account_id, currency, leasing_ids.debtor_id,
                name_sequence, case when (leasing_ids.order_date + interval '7 hour')::date < create_dates::date then 'posted' else 'draft' end, concat(feetypeids.name, ' - ', channelids.name), 'product', 
                (leasing_ids.order_date + interval '7 hour')::date, leasing_ids.interest_to_eom, 0.00,
            ...
            INSERT INTO account_move_line(
                ...
                nextval('account_move_line_id_seq'), account_move_id, leasing_account_ids.account_journal_id, 1, currency, leasing_account_ids.credit_account_id, currency, leasing_ids.debtor_id,
                name_sequence, case when (leasing_ids.order_date + interval '7 hour')::date < create_dates::date then 'posted' else 'draft' end, concat(feetypeids.name, ' - ', channelids.name), 'product',
                (leasing_ids.order_date + interval '7 hour')::date, 0.00, leasing_ids.interest_to_eom,
...
DROP FUNCTION IF EXISTS create_journal_interest_journal(int, int);
CREATE OR REPLACE FUNCTION public.create_journal_interest_journal(line_id int, leas_account_id int)
RETURNS INT
LANGUAGE plpgsql AS
$$
    DECLARE
    ...
    BEGIN
        ...
        IF leasing_account_ids.id IS NOT NULL THEN
            ...
            INSERT INTO account_move_line(
                ...
                nextval('account_move_line_id_seq'), account_move_id, leasing_account_ids.account_journal_id, 1, currency, leasing_account_ids.debit_account_id, currency, leasing_ids.debtor_id,
                name_sequence, case when (leasing_ids.order_date + interval '7 hour')::date < create_dates::date then 'posted' else 'draft' end, concat(feetypeids.name, ' - ', channelids.name), 'product', 
                (leasing_ids.order_date + interval '7 hour')::date, leasing_ids.interest_to_due_date, 0.00,
            ...
            INSERT INTO account_move_line(
                ...
                nextval('account_move_line_id_seq'), account_move_id, leasing_account_ids.account_journal_id, 1, currency, leasing_account_ids.credit_account_id, currency, leasing_ids.debtor_id,
                name_sequence, case when (leasing_ids.order_date + interval '7 hour')::date < create_dates::date then 'posted' else 'draft' end, concat(feetypeids.name, ' - ', channelids.name), 'product',
                (leasing_ids.order_date + interval '7 hour')::date, 0.00, leasing_ids.interest_to_due_date,
...
DROP FUNCTION IF EXISTS create_journal_amortization(int, int);
CREATE OR REPLACE FUNCTION public.create_journal_amortization(line_id int, feetype int)
RETURNS INT
LANGUAGE plpgsql AS
$$
    DECLARE
    ...
    BEGIN
        ...
        SELECT mft.amortization_account_journal_id as journalid, mla.debit_account_amortization_id as debitacc, mla.credit_account_amortization_id as creditacc, aj.*
            ...
            WHERE mft.id = feetype
                ...
                AND mla.is_restructure = leasing_ids.is_restructure
                AND mla.active = true INTO leasing_account_ids;
        ...
        IF leasing_account_ids.journalid IS NOT NULL THEN
            ...
            name_sequence := get_sequence_name_journal(leasing_account_ids.journalid, (amortization_ids.order_date + interval '7 hour')::varchar);

            SELECT * FROM mofi_fee_type WHERE id = amortization_ids.fee_type_id INTO feetypeids;
            SELECT * FROM mofi_channel WHERE id = amortization_ids.channel_id INTO channelids;
            ...
            INSERT INTO account_move(
                ...
                nextval('account_move_id_seq'), name_sequence, amortization_ids.debtor_id, (amortization_ids.order_date  + interval '7 hour')::date, leasing_account_ids.journalid,
                2, 1, currency, concat(leasing_account_ids.code, '/', date_part('year', (amortization_ids.order_date  + interval '7 hour')::date), '/', date_part('month', (amortization_ids.order_date + interval '7 hour')::date), '/'),
                'entry', '', 0.00, 0.00, amortization_ids.amount,
                0.00, 0.00, amortization_ids.amount, 0.00,
                (amortization_ids.order_date  + interval '7 hour')::date, case when (amortization_ids.order_date + interval '7 hour')::date < create_dates::date then 'posted' else 'draft' end,
                'not_paid', case when (amortization_ids.order_date + interval '7 hour')::date < create_dates::date then 'no' else 'at_date' end,
                'no_extract_requested', 2, create_dates, 2, create_dates,
            ...
            INSERT INTO account_move_line(
                ...
                name_sequence, case when (amortization_ids.order_date + interval '7 hour')::date < create_dates::date then 'posted' else 'draft' end, concat('Amortization - ', feetypeids.name, ' - ', channelids.name), 'product',
                (amortization_ids.order_date + interval '7 hour')::date, amortization_ids.amount, 0.00,
            ...
            INSERT INTO account_move_line(
                ...
                name_sequence, case when (amortization_ids.order_date + interval '7 hour')::date < create_dates::date then 'posted' else 'draft' end, concat('Amortization - ', feetypeids.name, ' - ', channelids.name), 'product',
                (amortization_ids.order_date + interval '7 hour')::date, 0.00, amortization_ids.amount,
