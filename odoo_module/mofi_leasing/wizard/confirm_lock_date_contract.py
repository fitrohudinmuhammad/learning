...
class MofiConfirmLockDate(models.TransientModel):
    _name = 'mofi.confirm.lock.date'
    ...
    def default_get(self, fields):
        res = super(MofiConfirmLockDate, self).default_get(fields)
        if self._context.get('active_id'):
            res['contract_id'] = self._context.get('active_id')
        return res

    def button_proceed(self):
        for record in self.contract_id:
            _log.error('account_move_id: %s', record.name)
            ...
            if lock_check and date_check and self.journal_date <= date_check:
                ...
                # self.contract_id.generate_amortization()
                self.contract_id.write({'status': 'active'})
