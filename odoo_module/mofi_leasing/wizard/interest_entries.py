...
class MofiInterestEntries(models.TransientModel):
    _name = 'mofi.interest.entries'

    @api.model
    def default_get(self, fields):
        res = super(MofiInterestEntries, self).default_get(fields)
    ...
    def button_proceed_interest(self):
        _log.info('Process Generate Journal Interest')
        for rec in self.contract_id:
            ...
            if lock_check and date_check and self.journal_date <= date_check:
                ...
                # rec.with_context(journal_date=self.journal_date, date_check=True).create_interest_journal_entry()
                self.env.cr.execute("""SELECT generate_interest_lock_date(%s, '%s')""" % (self.contract_id.id, self.journal_date))
                self.env.cr.commit()
